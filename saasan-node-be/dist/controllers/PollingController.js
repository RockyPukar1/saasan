...
class PollingController {
    ...
    static async getAnalytics(req, res) {
        try {
            ...
            res.json(ResponseHelper_1.ResponseHelper.success(analytics));
        }
        catch (error) {
            console.error("Get analytics error:", error);
            res.status(500).json(ResponseHelper_1.ResponseHelper.error("Failed to fetch analytics"));
        }
    }
    static async getCategories(req, res) {
        try {
            ...
            res.json(ResponseHelper_1.ResponseHelper.success(categories));
        }
        catch (error) {
            console.error("Get categories error:", error);
            res.status(500).json(ResponseHelper_1.ResponseHelper.error("Failed to fetch categories"));
        }
    }
    static async getPoliticianComparison(req, res) {
        try {
            const { politicianId } = req.params;
            const comparison = await PollModel_1.PollModel.getPoliticianComparison(politicianId);
            res.json(ResponseHelper_1.ResponseHelper.success(comparison));
        }
        catch (error) {
            console.error("Get politician comparison error:", error);
            res
                .status(500)
                .json(ResponseHelper_1.ResponseHelper.error("Failed to fetch politician comparison"));
        }
    }
    static async getPartyComparison(req, res) {
        try {
            const { partyId } = req.params;
            const comparison = await PollModel_1.PollModel.getPartyComparison(partyId);
            res.json(ResponseHelper_1.ResponseHelper.success(comparison));
        }
        catch (error) {
            ...
            res
                .status(500)
                .json(ResponseHelper_1.ResponseHelper.error("Failed to fetch party comparison"));
        }
    }
}
